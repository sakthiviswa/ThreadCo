...
const KPI_ITEMS = [
  { label: 'Total Revenue', value: '₹1,24,500', change: '+12%', up: true, Icon: TrendingUp },
  { label: 'Total Orders', value: '693', change: '+8%', up: true, Icon: ShoppingBag },
  { label: 'Avg Order Value', value: '₹1,796', change: '+4%', up: true, Icon: BarChart2 },
  { label: 'Return Rate', value: '2.3%', change: '-0.5%', up: false, Icon: RefreshCw },
  { label: 'Conversion Rate', value: '3.8%', change: '+0.2%', up: true, Icon: MousePointerClick },
  { label: 'Avg Rating', value: '4.6', change: '+0.1', up: true, Icon: Star },
]
    ...
    <div className="analytics-page">
      <div className="container">
        ...
        <div className="kpi-grid">
          {KPI_ITEMS.map(({ label, value, change, up, Icon }) => (
            <div key={label} className="kpi-card card">
              <div className="kpi-label" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <Icon size={14} strokeWidth={1.8} style={{ color: 'var(--mid)' }} />
                {label}
              </div>
              <div className="kpi-value">{value}</div>
              <div className={`kpi-change ${up ? 'up' : 'down'}`}>
                {up
                  ? <TrendingUp size={12} style={{ marginRight: 3, verticalAlign: 'middle' }} />
                  : <TrendingDown size={12} style={{ marginRight: 3, verticalAlign: 'middle' }} />
                }
                ...
                <div key={m.month} className="bar-col">
                  ...
                  <div className="bar-wrap">
                    <div
                      ...
                      style={{ height: `${(m.orders / Math.max(...MONTHLY.map(x => x.orders))) * 100}%` }}
                    />
                  </div>
                  <div className="bar-label">{m.month}</div>
                </div>
        ...
        <div className="card" style={{ padding: 22, marginTop: 20 }}>
          ...
          <table className="sp-table">
            <thead>
              <tr>
                <th>#</th><th>Product</th><th>Revenue</th>
                <th>Orders</th><th>Rating</th><th>Revenue Share</th>
              </tr>
            </thead>
            <tbody>
                ...
                return (
                  <tr key={p.name}>
                    ...
                    <td>
                      <Star
                        size={13}
                        style={{ verticalAlign: 'middle', marginRight: 3, color: '#f59e0b', fill: '#f59e0b' }}
                      />
                      {p.rating}
                    </td>
                    <td>
